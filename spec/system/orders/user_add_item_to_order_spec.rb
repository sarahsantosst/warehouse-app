...
describe 'Usuário adiciona itens ao pedido' do 
  it ' com sucesso' do
    #arrange
    user = User.create(name: 'João Almeida',
   ...
   it 'e não vê produtos de outro fornecedor' do
    ...
    user = User.create(name: 'João Almeida',
      ...
      cep: '15000-000',
      description: 'Galpão destinado para cargas internacionais')
      supplier_a = Supplier.create!(corporate_name:'ACME LTDA',
      brand_name: 'ACME', 
      registration_number:'4344721600765', 
      full_address:'Av das Palmas, 100', 
      city:'Bauru', 
      state:'SP', 
      email: 'contato@acme.com')
      supplier_b = Supplier.create!(corporate_name: 'SIB-LTDA', 
        brand_name: 'SIB', 
      ...
      product_a = ProductModel.create!(name: 'Produto A', weight: 15, width: 10,  height: 20, depth: 30, supplier: supplier_a, sku: 'PRODUTOA')
  
      product_b = ProductModel.create!(name: 'Produto B', weight: 15, width: 10,  height: 20, depth: 30, supplier: supplier_b, sku: 'PRODUTOB')
    #act
    login_as user
    visit root_path
    click_on 'Meus Pedidos'
